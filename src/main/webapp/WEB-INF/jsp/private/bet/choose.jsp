...
<body>
    ...
    <div id="results">
        <h3 id="resultTitle">
            Bet a tournament! <a class="resultTitleItem" href="/hello.htm">View
                messages</a>
            <div id="test"></div>
        </h3>
        <c:forEach items="${tournamentList}" var="tournament" varStatus="status">
            <div class="message">
                ...
                <a class="link"
                    href="javascript:bringHTML('${links[status.index]}','bets${tournament.id}', 'collapseTop${tournament.id}')">Bet</a>
                <a class="link"
                    href="javascript:bringHTML('${see[status.index]}','bets${tournament.id}', 'collapseTop${tournament.id}')">See
                    ranking</a> <a class="link"
                    href="javascript:collapse('bets${tournament.id}', '${tournament.id}')"
                    id="collapseTop${tournament.id}"></a>
                <div class="betContainer" id="bets${tournament.id}"></div>
                <div class="ranking" id="ranking${tournament.id}"></div>
            </div>
        </c:forEach>
        <div id="hiddenColor" />
    </div>
